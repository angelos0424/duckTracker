...
import type { AddDownloadDialogMode, FormatOption } from '../types.js';

export interface AddDownloadDialogProps {
    open: boolean;
    mode: AddDownloadDialogMode;
    urlValue: string;
    errorMessage: string;
...
export const AddDownloadDialog: FC<AddDownloadDialogProps> = ({
    ...
    const handleSelectFormat = onSelectFormat ?? (() => {});

    return (
        <div
            className={`dialog-backdrop${open ? ' visible' : ''}`}
